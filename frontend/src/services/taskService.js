...
export function executeTaskNow(id) {
  return request({
    url: `/api/tasks/${id}/execute/`,  // 后端立即执行任务接口
    method: 'post'
  })
}
...
export function getTaskResults(params) {
  return request({
    url: '/api/task-results/',  // 后端任务执行结果接口
    method: 'get',
    params  // 任务ID、分页等参数
  })
}
